...
`ts-jest` by default does **NOT** use Babel. But you may want to use it, especially if your code relies on Babel plugins to make some transformations. `ts-jest` can call the BabelJest processor once TypeScript has stripped the type annotations from the source code.
